<?php
	...
	if( $this->param('fbinfo') && isset($_SESSION[$this->param('fbinfo')]) ) {
		...
		if( isset($fbinfo->uid, $fbinfo->username, $fbinfo->name) ) {
			...
			$fbinfo->proxied_email	= $fbinfo->proxied_email=='null'||$fbinfo->proxied_email=='undefined' ? '' : $fbinfo->proxied_email;
		}
	}
	$twinfo	= FALSE;
	...
	$ldapinfo	= FALSE;
	if( $this->param('ldapinfo') && isset($_SESSION['LDAP_'.$this->param('ldapinfo')]) && isset($C->LDAP_ON) && $C->LDAP_ON ) {
		$ldapinfo	= $_SESSION['LDAP_'.$this->param('ldapinfo')];
		$ldapinfo->tmp_valid	= TRUE;
		$C->USERS_EMAIL_CONFIRMATION	= FALSE;
	}
	...
	elseif( ($C->USERS_EMAIL_CONFIRMATION && $this->param('regid') && $this->param('regkey')) || !$C->USERS_EMAIL_CONFIRMATION )
	{
		...
		if( $ldapinfo && isset($ldapinfo->tmp_valid) && $ldapinfo->tmp_valid ) {
			$D->email	= $ldapinfo->email;
			$D->username	= $ldapinfo->username;
			$D->password	= $ldapinfo->password;
			$D->password2	= $ldapinfo->password;
			$D->fullname	= $ldapinfo->fullname;
		}
